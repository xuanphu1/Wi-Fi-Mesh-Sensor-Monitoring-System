...
\begin{itemize}
    \item \textbf{Ban quản lý khu công nghiệp, nhà máy:} Theo dõi sát sao các chỉ số môi trường (nhiệt độ, nồng độ bụi mịn) để đảm bảo an toàn lao động và vận hành máy móc.
    \item \textbf{Nông nghiệp công nghệ cao:} Giám sát các chỉ số vi khí hậu trong nhà kính, trang trại thông minh, làm cơ sở dữ liệu để tự động hóa hệ thống tưới tiêu và thông gió.
    \item \textbf{Quản trị viên hệ thống (System Admin):} Nắm bắt trực quan "sức khỏe" của toàn bộ mạng lưới thiết bị (Node/Gateway) cũng như tài nguyên máy chủ thông qua biểu đồ thời gian thực.
\end{itemize}
...
\textbf{Frontend (Mobile Application):}
\begin{itemize}
    \item \textbf{React Native \& Expo:} Xây dựng khung giao diện UI đa nền tảng (iOS và Android) với hiệu năng mượt mà.
    \item \textbf{Zustand:} Hệ thống quản lý State toàn cục siêu nhẹ, tối ưu hóa quá trình re-render khi xử lý dữ liệu cảm biến (Live Stream).
    \item \textbf{TailwindCSS \& Reanimated:} Thiết kế giao diện Glassmorphism hiện đại và tạo chuyển động mượt mà cho đồ thị, thông báo.
\end{itemize}

\textbf{Backend Services \& Infrastructure:}
\begin{itemize}
    \item \textbf{Node.js \& WebSockets:} Triển khai luồng truyền phát dữ liệu hai chiều (Full-duplex) theo thời gian thực giữa Gateway và Mobile App.
    \item \textbf{HTTP API (RESTful):} Cung cấp cơ chế truy vấn (GET) an toàn để trích xuất dữ liệu lịch sử từ máy chủ.
    \item \textbf{Nginx Reverse Proxy:} Hệ thống phân giải luồng truy cập trên Cloud, chạy song song giao thức HTTPS và WSS an toàn trên cùng Port 443.
\end{itemize}

\textbf{Database \& Data Storage:}
\begin{itemize}
    \item \textbf{Cơ sở dữ liệu Máy chủ (SQLite):} Sử dụng hệ quản trị CSDL siêu nhẹ SQLite để đảm nhiệm việc lưu trữ cấu hình mạng lưới và hàng triệu bản ghi dữ liệu chuỗi thời gian (Time-series data) từ các node cảm biến đẩy lên liên tục.
    \item \textbf{AsyncStorage \& Expo FileSystem:} Áp dụng ở phía Client (Mobile App) để lưu trữ vĩnh viễn các cấu hình IP, ngưỡng cảnh báo (Threshold) và xuất báo cáo dữ liệu định dạng CSV trực tiếp từ bộ nhớ điện thoại.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|l|}
        ...
        \hline
        \multirow{2}{*}{History} & Truy vấn dữ liệu & Lọc dữ liệu theo thời gian, cảm biến cụ thể và vẽ biểu đồ lịch sử kép. & H. Xuân Phú \\ \cline{2-4}
        ...
        \hline
    \end{tabularx}
    \caption{Bảng phân rã chức năng hệ thống và phân công nhiệm vụ}
\end{table}
...
\textbf{Cấu trúc 3 loại bản tin (Packet) truyền tải trong hệ thống:}
\begin{itemize}
    \item \textbf{Bản tin Cảm biến (\texttt{uart\_tx/rx}):} Đóng gói toàn bộ các thông số đo lường môi trường (Nhiệt độ, Độ ẩm, Bụi mịn, v.v.) thu thập từ các Node. Bản tin bao gồm địa chỉ MAC thiết bị, phiên bản Firmware, thời gian thực (RTC) và danh sách mảng các cổng (\texttt{ports}) mang các cặp \texttt{key-value} giá trị đo lường.
\begin{verbatim}
{
  "type": "uart_rx",
  "payload": {
...
\end{verbatim}
    \item \textbf{Bản tin Gateway (\texttt{gateway\_status}):} Mang dữ liệu chẩn đoán trạng thái phần cứng của Trạm trung tâm Gateway. Cung cấp thông số phần cứng bao gồm \% tải CPU, lượng RAM khả dụng, điện áp pin (V), \% pin còn lại, thông tin mạng Wi-Fi (SSID, IP, RSSI) và thời gian sống \texttt{uptime}.
\begin{verbatim}
{
  "type": "gateway_status",
...
\end{verbatim}
    \item \textbf{Bản tin Máy chủ (\texttt{server\_metrics}):} Cung cấp số liệu "sức khỏe" của Backend Server trên Cloud. Đo lường liên tục mức độ tiêu thụ tài nguyên của Node.js Backend bao gồm tải CPU, dung lượng RAM (Total/Used), nhiệt độ máy chủ và thời gian duy trì kết nối.
\begin{verbatim}
{
  "type": "server_metrics",
  "cpuLoadPercent": 5.2,
...
\vspace{0.3cm}
\noindent\textbf{Video Demo Hệ thống:} \href{https://drive.google.com/drive/folders/1Sm_5AlTHUcPrvPvF8nTApXCBocLFyrjh?hl=vi}{\underline{Nhấn vào đây để xem video minh họa toàn bộ hoạt động thực tế}}
\vspace{0.3cm}
...
\newpage
\section*{PHỤ LỤC: BÁO CÁO CÁ NHÂN}
\addcontentsline{toc}{section}{\numberline{}PHỤ LỤC: BÁO CÁO CÁ NHÂN - HỒ XUÂN PHÚ}

\subsection*{1. Thông tin cá nhân và vai trò}
\begin{itemize}
    \item \textbf{Họ và tên:} Hồ Xuân Phú
    \item \textbf{Vai trò trong nhóm:}Phụ trách xây dựng kiến trúc Backend Server, cơ sở dữ liệu và phát triển các chức năng chính trên Ứng dụng di động.
\end{itemize}

\subsection*{2. Khối chức năng/Phần tích hợp chịu trách nhiệm chính}
\begin{itemize}
    \item \textbf{Kiến trúc Backend \& Mạng lưới:} Thiết kế và xây dựng máy chủ trung tâm (Backend Server), cấu hình hệ thống WebSockets để truyền tải dữ liệu thời gian thực và thiết lập Nginx Reverse Proxy.
    \item \textbf{Frontend - Dashboard (Trang chủ):} Xây dựng trung tâm kiểm soát, tính toán số lượng Node, hiển thị trạng thái tổng quan.
    \item \textbf{Frontend - Monitor Server:} Xây dựng màn hình giám sát sức khỏe của máy chủ (CPU, RAM).
    \item \textbf{Frontend - History (Lịch sử) \& Xuất báo cáo:} Lập trình chức năng truy vấn dữ liệu theo mốc thời gian, vẽ biểu đồ lịch sử kép và thuật toán xử lý hàng ngàn bản ghi để tạo file CSV cục bộ.
\end{itemize}

\subsection*{3. Danh sách file, Component, API và Kỹ thuật trực tiếp thực hiện}
\begin{itemize}
    \item \textbf{Màn hình (Screens):} \path{app/(tabs)/index.tsx}, \path{app/(tabs)/server.tsx}, \path{app/(tabs)/history.tsx}.
    \item \textbf{Dịch vụ \& API:} Xây dựng toàn bộ module \path{src/services/modules/history.ts} để giao tiếp REST API với Backend.
    \item \textbf{Công nghệ \& Kỹ thuật:} Sử dụng Zustand State Management, WebSockets, React Native Expo, thuật toán xử lý mảng (Array processing) để bóc tách dữ liệu JSON từ Node và vẽ biểu đồ.
\end{itemize}

\subsection*{4. Mô tả vấn đề gặp phải và Cách giải quyết}
\begin{itemize}
    \item \textbf{Vấn đề 1 (Quá tải bộ nhớ khi xem lịch sử):} Khi truy vấn dữ liệu 7 ngày, lượng dữ liệu JSON trả về lên tới hàng ngàn bản ghi, làm ứng dụng bị treo (freeze) khi cố gắng vẽ biểu đồ. 
    \item \textbf{Cách giải quyết:} Áp dụng Hook \texttt{useMemo} trong React để ghi nhớ kết quả tính toán, đồng thời thực hiện rút gọn mảng (downsampling) ở Client trước khi truyền vào thư viện vẽ đồ thị, giúp UI mượt mà trở lại.
    \item \textbf{Vấn đề 2 (Đồng bộ dữ liệu Realtime):} Mất đồng bộ giữa trạng thái cảnh báo và giao diện khi gói tin WebSockets đổ về với tần số quá cao.
    \item \textbf{Cách giải quyết:} Tập trung hóa luồng dữ liệu vào kho chứa chung (\path{useMeshStore.ts}), xử lý tối ưu để giảm tần suất Re-render giao diện dư thừa.
\end{itemize}

\subsection*{5. Tự đánh giá mức độ hoàn thành}
\begin{itemize}
    \item \textbf{Mức độ hoàn thành công việc:} Đạt 100\% (Hoàn thiện toàn bộ kiến trúc Backend và các màn hình Frontend được giao, App chạy cực kỳ ổn định).
    \item \textbf{Mức độ hiểu sản phẩm chung:} Rất hiểu (Nắm vững từ kiến trúc phần cứng ESP32 truyền lên Gateway, luồng dữ liệu qua WebSockets, cho đến cách Frontend bóc tách và vẽ lên giao diện).
\end{itemize}
