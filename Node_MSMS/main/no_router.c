 ...
 * - p: danh sách port sensor; mỗi phần tử có dạng [port_1based, sensor_type,
 * value0, value1, ...].
 *
 * Ví dụ:
...
#define DS3231_I2C_SDA_GPIO GPIO_NUM_21
#define DS3231_I2C_SCL_GPIO GPIO_NUM_22
 ...
 * - Each p item starts with port_1based and SensorType_t wire ID, followed by
 * sensor-specific float values.
...
static void mesh_fill_ds3231_time_str(char *out, size_t out_sz) {
  ...
  esp_err_t err = i2c_master_write_read_device(
      DS3231_I2C_PORT, DS3231_I2C_ADDR, &start_reg, sizeof(start_reg), raw,
      sizeof(raw), pdMS_TO_TICKS(50));
  if (err != ESP_OK) {
  ...
  snprintf(out, out_sz, "%04d-%02d-%02dT%02d:%02d:%02d", year, mon, day, hour,
           min, sec);
}

static size_t mesh_build_udp_telemetry_json(char *payload, size_t payload_sz,
                                            uint32_t seq) {
  ...
  if (mesh_build_error_codes_json_array(err_codes_json,
                                        sizeof(err_codes_json)) == 0) {
    return 0;
  }
   ...
   * - t: thời gian lấy từ DS3231 (fallback "1970-01-01T00:00:00" nếu chưa đọc
   * được RTC)
   */
  int written =
      snprintf(payload, payload_sz,
               "{\"v\":%d,\"seq\":%" PRIu32
               ",\"n\":%d,\"i\":\"%s\",\"t\":\"%s\",\"ver\":\"%s\",\"err\":%s,"
               "\"p\":[[1,0,%.3f,%.3f,%.3f],[3,13,%.3f,%.3f]]}",
               MESH_UDP_JSON_SCHEMA, seq, node_level, sta_ip, rtc_time,
               MESH_NODE_VERSION, err_codes_json, bme[0], bme[1], bme[2],
               aht10[0], aht10[1]);
...
static void mesh_udp_client_task(void *pvParameters) {
  ...
  while (1) {
    ...
    if (mesh_level <= 1) {
      ESP_LOGW(TAG, "Mesh is not joined yet (layer=%d), skip UDP telemetry",
               mesh_level);
    ...
    size_t plen =
        mesh_build_udp_telemetry_json(payload, sizeof(payload), next_seq);
    if (plen == 0) {
    ...
    int err = sendto(sock, payload, plen, 0, (struct sockaddr *)&dest_addr,
                     sizeof(dest_addr));
...
static void print_system_info_timercb(TimerHandle_t timer) {
  ...
  for (uint32_t loop = 0; (loop < size) && (node != NULL); loop++) {
    ...
    printf("%u: %d, " MACSTR ", %s\r\n", (unsigned int)(loop + 1),
           node->node->level, MAC2STR(node->node->mac_addr),
           inet_ntoa(ip_struct));
    node = node->next;
  }
}
...
void app_main() {
  ...
  if (i2c_err != ESP_OK) {
    ESP_LOGW(TAG, "i2c_param_config failed (%s), skip DS3231",
             esp_err_to_name(i2c_err));
  } else {
    i2c_err = i2c_driver_install(DS3231_I2C_PORT, I2C_MODE_MASTER, 0, 0, 0);
    if (i2c_err != ESP_OK) {
      ESP_LOGW(TAG, "i2c_driver_install failed (%s), skip DS3231",
               esp_err_to_name(i2c_err));
    } else {
      s_ds3231_ready = true;
  ...
  if (led_err != ESP_OK) {
    ESP_LOGW(TAG,
             "Không khởi tạo được LED RGB (%s) — tiếp tục không báo hiệu đèn",
