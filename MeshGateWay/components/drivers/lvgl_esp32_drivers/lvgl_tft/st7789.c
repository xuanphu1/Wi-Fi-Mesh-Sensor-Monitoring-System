...
void st7789_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_map)
{
    ...
    offsety2 += CONFIG_LV_TFT_DISPLAY_Y_OFFSET;
        ...
        offsety2 += 40;
    #endif
#elif (LV_HOR_RES_MAX == 320) && (LV_VER_RES_MAX == 172) // 1.9 inch 170×320 LCD, physically landscape
    #if (CONFIG_LV_DISPLAY_ORIENTATION_PORTRAIT) || (CONFIG_LV_DISPLAY_ORIENTATION_PORTRAIT_INVERTED)
        offsety1 += 34;
        offsety2 += 34;
    #endif
#elif (LV_HOR_RES_MAX == 172) && (LV_VER_RES_MAX == 320) // 1.9 inch 170×320 LCD, physically vertical
    ...
    //printf("Colors map: %d, size: %d\n", (int)color_map[0].full, size);
...
static void st7789_send_color(void * data, size_t length)
{
    //printf("Data color: %02x, length: %d\n", *(uint16_t *)data, length);
    disp_wait_for_pending_transactions();
    gpio_set_level(ST7789_DC, 1);
