...
#define DS3231_CFG_I2C_PORT CONFIG_DS3231_I2C_PORT
#else
#define DS3231_CFG_I2C_PORT 0
#endif

#ifdef CONFIG_DS3231_SDA_GPIO
#define DS3231_CFG_SDA_GPIO CONFIG_DS3231_SDA_GPIO
#else
#define DS3231_CFG_SDA_GPIO 21
#endif

#ifdef CONFIG_DS3231_SCL_GPIO
#define DS3231_CFG_SCL_GPIO CONFIG_DS3231_SCL_GPIO
#else
#define DS3231_CFG_SCL_GPIO 22
#endif
...
esp_err_t ds3231_init_default(i2c_dev_t *dev)
{
    CHECK_ARG(dev);
    ...
    if (ret != ESP_OK)
    {
        printf("[RTC ] i2cdev_init failed: %s (0x%x)\n", esp_err_to_name(ret), (unsigned)ret);
        return ret;
    }

    ret = ds3231_init_desc(dev, DS3231_CFG_I2C_PORT, DS3231_CFG_SDA_GPIO, DS3231_CFG_SCL_GPIO);
    if (ret != ESP_OK)
    {
        printf("[RTC ] ds3231_init_desc failed: %s (0x%x)\n", esp_err_to_name(ret), (unsigned)ret);
        return ret;
    }

    struct tm t;
    ret = ds3231_get_time(dev, &t);
    if (ret != ESP_OK)
    {
        printf("[RTC ] ds3231_get_time failed: %s (0x%x)\n", esp_err_to_name(ret), (unsigned)ret);
        return ret;
    }
...
// 2020-04-16T18:34:56
void ds3231_get_time_str(struct tm *time, char *buf, size_t buf_size)
{
...
const char *getWeekDay(uint8_t weekDay)
{
    switch (weekDay)
    {
    case 0:
