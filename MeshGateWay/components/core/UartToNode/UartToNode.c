...
/** Dòng ASCII thuần “No node” / “No Node” từ mesh (không JSON) — chỉ life, không uplink. */
...
/** Bản tin chỉ để “check life” — không đẩy uplink (vẫn xử lý RX/handshake bình thường). */
static bool uart_rx_is_no_node_only_life(const uint8_t *data, size_t len)
{
    ...
    /*
     * JSON telemetry kiểu {"v":1,...} không có khóa "type" — tránh malloc+cJSON_Parse
     * mỗi chunk (dễ cạn heap và làm HTTP captive/SPIFFS fail).
     */
...
static void uart_node_enqueue_rx(const uint8_t *data, size_t len)
{
    ...
    while (len > 0) {
        ...
        if (xQueueSend(s_uplink_queue, &item, 0) != pdTRUE) {
            ESP_LOGW(TAG, "uart uplink queue full, drop %u bytes", (unsigned)item.len);
        }
...
static void uart_to_node_handshake_ok(uart_to_node_ctx_t *ctx)
{
    ...
    ctx->hs_fill = 0;
    ESP_LOGI(TAG, "Handshake OK (exact wire): Switching to root mode... -> connected");
}
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (c == '\n') {
            if (s_rx_line_len > 0) {
                ...
                ESP_LOGI(TAG, "RX line: %s", s_rx_line);
                s_rx_line_len = 0;
            }
            continue;
        }
...
static void uart_to_node_drain_rx_hw_fifo(uart_to_node_ctx_t *ctx, uart_port_t u)
{
    uint8_t tmp[256];
    size_t buffered = 0;
...
static esp_err_t uart_to_node_uart_init(void)
{
    ...
    /* RX lớn + queue sự kiện: driver tạo queue, ISR đẩy uart_event_t (UART_DATA, overflow, ...) */
    ESP_RETURN_ON_ERROR(uart_driver_install(u, 8192, 2048, 24, &s_uart_queue, 0), TAG, "uart_driver_install");
    ESP_RETURN_ON_ERROR(uart_param_config(u, &uart_config), TAG, "uart_param_config");
    ESP_RETURN_ON_ERROR(uart_set_pin(u, cfg_tx_pin(), cfg_rx_pin(), cfg_rts_pin(), cfg_cts_pin()), TAG, "uart_set_pin");

    uart_intr_config_t uart_intr = {
        .intr_enable_mask = UART_TO_NODE_INTR_RXFIFO_FULL | UART_TO_NODE_INTR_RXFIFO_TOUT,
        .rxfifo_full_thresh = 100,
        .txfifo_empty_intr_thresh = 10,
        .rx_timeout_thresh = 10,
    };
    ESP_RETURN_ON_ERROR(uart_intr_config(u, &uart_intr), TAG, "uart_intr_config");
    ...
    while (1) {
        ...
        if (xQueueReceive(s_uart_queue, &event, queue_wait) == pdTRUE) {
            ...
            case UART_DATA: {
                ...
                uint8_t tmp[256];
                if (remaining == 0) {
                    uart_to_node_drain_rx_hw_fifo(&ctx, u);
                    break;
                }
                while (remaining > 0) {
        ...
        /* Dự phòng: đọc nốt FIFO nếu còn sót byte (ít gặp khi đã UART_DATA) */
        uart_to_node_drain_rx_hw_fifo(&ctx, u);
...
esp_err_t uart_to_node_start(void)
{
    ...
    BaseType_t ok = xTaskCreate(uart_to_node_task, "uart_to_node", (uint32_t)stack_words, NULL, prio, NULL);
