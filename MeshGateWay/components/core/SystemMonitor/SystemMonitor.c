...
static uint32_t cpu_idle_permille_runtime_stats(dm_cpu_t *cpu)
{
    ...
    TaskStatus_t *task_status = (TaskStatus_t *)malloc(sizeof(TaskStatus_t) * (task_count + 4));
    if (task_status == NULL)
        return 0;

    uint32_t total_runtime = 0;
    UBaseType_t got = uxTaskGetSystemState(task_status, task_count + 4, &total_runtime);
    if (got == 0)
    {
        free(task_status);
        return 0;
...
static void system_monitor_task(void *arg)
{
    ...
    while (1)
    {
        ...
        if (ctx->hw->rtc_ready && ds3231_get_time(&ctx->hw->rtc_dev, &m.rtc_time) == ESP_OK)
            m.rtc_ok = true;

        m.battery_pct = power_manager_battery_get_percent(ctx->hw, &bat_raw_avg, &bat_adc_mv);
        if (ctx->hw->adc_ready && bat_adc_mv > 0) {
            m.battery_pack_mv = (uint32_t)((uint64_t)bat_adc_mv *
                                           (BAT_R_TOP_OHMS + BAT_R_BOTTOM_OHMS) /
                                           BAT_R_BOTTOM_OHMS);
        } else {
            m.battery_pack_mv = 0;
        }
        ...
        {
            m.sd_used_percent = (uint32_t)(((m.sd_total_kb - m.sd_free_kb) * 100ULL) / m.sd_total_kb);
        }

        {
            ...
            m.ram_free_percent = (total > 0) ? (uint32_t)((free * 100ULL) / total) : 0;
            m.ram_free_permille = (total > 0) ? (uint32_t)((free * 1000ULL) / total) : 0;
        ...
        if (!logged_no_tsens) {
            ...
            ESP_LOGW(TAG,
                     "Nhiệt chip nội bộ: target không có SOC_TEMP_SENSOR trong IDF (vd. ESP32); "
                     "gateway_status gửi chip_temp_c=null — dùng C3/S3/C6… hoặc cảm biến ngoài.");
        }
#endif

        if (m.rtc_ok) {
            (void)snprintf(ctx->telemetry->timestamp, sizeof(ctx->telemetry->timestamp),
                             "%04d-%02d-%02dT%02d:%02d:%02d",
                             (int)(m.rtc_time.tm_year + 1900),
                             (int)(m.rtc_time.tm_mon + 1),
                             (int)m.rtc_time.tm_mday,
                             (int)m.rtc_time.tm_hour,
                             (int)m.rtc_time.tm_min,
                             (int)m.rtc_time.tm_sec);
        } else {
            ctx->telemetry->timestamp[0] = '\0';
        }

        if (ctx->metrics->mutex && xSemaphoreTake(ctx->metrics->mutex, pdMS_TO_TICKS(20)) == pdTRUE)
        {
            ctx->metrics->value = m;
            xSemaphoreGive(ctx->metrics->mutex);
        }

        if ((uint32_t)(now_ms - last_log_ms) >= 5000)
        {
            last_log_ms = now_ms;
            // printf("[SENS] fps=%lu cpu_load=%lu.%lu sd_used=%lu bat_raw=%lu bat_adc_mv=%lu\n",
            //        (unsigned long)m.fps,
            //        (unsigned long)(m.cpu_load_permille / 10U),
            //        (unsigned long)(m.cpu_load_permille % 10U),
            //        (unsigned long)m.sd_used_percent,
            //        (unsigned long)bat_raw_avg,
            //        (unsigned long)bat_adc_mv);
        }
...
void system_monitor_start(dm_hw_t *hw,
                          ...
                          BaseType_t core_id)
{
    if (hw == NULL || cpu == NULL || lvgl == NULL || metrics == NULL || telemetry == NULL)
        return;
    ...
    xTaskCreatePinnedToCore(system_monitor_task, "system_monitor", 6144, &s_ctx, priority, NULL, core_id);
}
