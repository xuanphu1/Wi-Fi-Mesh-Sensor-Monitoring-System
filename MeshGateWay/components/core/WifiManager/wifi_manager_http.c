...
/** Triển khai trong WebSocKetHandle — tránh REQUIRES vòng CMake. */
...
static esp_err_t send_spiffs_file(httpd_req_t *req, const char *path, const char *content_type)
{
    FILE *file = fopen(path, "r");
    if (file == NULL) {
        ESP_LOGE(WIFI_TAG, "Failed to open %s", path);
    ...
    while (!feof(file)) {
        ...
        if (read_len > 0) {
            ...
            if (err != ESP_OK) {
                ESP_LOGE(WIFI_TAG, "Failed to send %s chunk: %s", path, esp_err_to_name(err));
                fclose(file);
                return err;
            }
        }
        if (ferror(file)) {
            ESP_LOGE(WIFI_TAG, "Failed to read %s", path);
            fclose(file);
            return ESP_FAIL;
        }
    }
...
esp_err_t wifi_config_handler(httpd_req_t *req)
{
    ...
    if (cJSON_IsString(ws_url_json)) {
        ...
        if (save_ws_url(ws_url) == ESP_OK) {
            ESP_LOGI(WIFI_TAG, "WebSocket URL set in RAM only (not flash; lost on reboot)");
        } else {
    ...
    if (cJSON_IsString(ssid_json) && cJSON_IsString(password_json)) {
        ...
        if (ws_url_json != NULL) {
            strncpy(message + msg_len, "WebSocket URL set in RAM (not flash). ", sizeof(message) - msg_len - 1);
...
esp_err_t wifi_status_handler(httpd_req_t *req)
{
    ...
    const bool sta_ap_ok = (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK);

    cJSON *root = cJSON_CreateObject();
    cJSON_AddBoolToObject(root, "ap_mode", ctx->ap_mode_active);
    cJSON_AddStringToObject(root, "ap_ssid", AP_SSID);
    cJSON_AddStringToObject(root, "ap_ip", ctx->ap_ip);
    cJSON_AddBoolToObject(root, "sta_connected", is_wifi_connected());
    cJSON_AddStringToObject(root, "sta_ssid", sta_ap_ok ? (const char *)ap_info.ssid : "");
    cJSON_AddStringToObject(root, "configured_ssid", ctx->configured_ssid);
    cJSON_AddStringToObject(root, "config_url", "http://meshgateway.local");
...
{
    return send_spiffs_file(req, "/spiffs/redirect.html", "text/html");
}
...
esp_err_t start_webserver(void)
{
    ...
    httpd_uri_t config_page_uri = {.uri = "/config",
                                   .method = HTTP_GET,
                                   .handler = root_handler,
                                   .user_ctx = NULL};
    httpd_register_uri_handler(ctx->server, &config_page_uri);

    httpd_uri_t setup_page_uri = {.uri = "/setup",
                                  .method = HTTP_GET,
                                  .handler = root_handler,
                                  .user_ctx = NULL};
    httpd_register_uri_handler(ctx->server, &setup_page_uri);
...
esp_err_t stop_webserver(void)
{
    ...
    if (ctx->dns_server_task_handle) {
        ...
        if (ctx->dns_server_task_handle) {
            vTaskDelete(ctx->dns_server_task_handle);
