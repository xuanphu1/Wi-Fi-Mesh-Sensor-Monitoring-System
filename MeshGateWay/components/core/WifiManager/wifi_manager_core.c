...
static bool wifi_try_sta_once(wifi_manager_ctx_t *ctx, const char *ssid, const char *password)
{
    ...
    xEventGroupClearBits(ctx->event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT | WIFI_STA_LINKED_BIT);
    esp_wifi_stop();
    ...
    EventBits_t bits = xEventGroupWaitBits(ctx->event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT,
...
static void wifi_manager_start_mdns(void)
{
    ...
    err = mdns_service_add("MeshGateway HTTP", "_http", "_tcp", 80,
                           service_txt,
                           sizeof(service_txt) / sizeof(service_txt[0]));
    if (err != ESP_OK) {
        ESP_LOGW(WIFI_TAG, "mDNS HTTP service add failed: %s", esp_err_to_name(err));
        return;
    }
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        xEventGroupClearBits(ctx->event_group, WIFI_CONNECTED_BIT | WIFI_STA_LINKED_BIT);
...
void wifi_init_sta(void)
{
    ...
    for (size_t i = 0; i < sizeof(s_boot_wifi_list) / sizeof(s_boot_wifi_list[0]); i++) {
        for (int attempt = 1; attempt <= 5; attempt++) {
            ESP_LOGI(WIFI_TAG, "Startup WiFi try %d/5 for SSID=%s", attempt, s_boot_wifi_list[i].ssid);
            if (wifi_try_sta_once(ctx, s_boot_wifi_list[i].ssid, s_boot_wifi_list[i].password)) {
                ESP_LOGI(WIFI_TAG, "Connected startup WiFi: SSID=%s", s_boot_wifi_list[i].ssid);
                connected = true;
                break;
            }
        }
    ...
    if (connected) {
        ESP_LOGI(WIFI_TAG, "Startup WiFi connected, keeping configuration AP active");
    } else {
        ESP_LOGI(WIFI_TAG, "All startup WiFi credentials failed, starting configuration AP");
    }
    if (xSemaphoreTake(ctx->mutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
        if (!connected) {
            esp_wifi_stop();
...
bool is_wifi_connecting(void)
{
    ...
    EventBits_t bits = xEventGroupGetBits(ctx->event_group);
    return !(bits & WIFI_STA_LINKED_BIT) &&
           !(bits & WIFI_AP_MODE_BIT) &&
           !(bits & WIFI_FAIL_BIT) &&
           ctx->connect_pending;
}
