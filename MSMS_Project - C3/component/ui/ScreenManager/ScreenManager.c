...
static system_err_t Draw_Menu_Frame(menu_list_t *menu, int8_t *selected,
  ...
  if (menu->count > 0) {
    ...
    for (int i = start_i; i <= end_i; i++) {
      ...
      if (draw_y > (int)offset - 16 && draw_y < 64) {
        ...
        uint8_t base_x =
            (menu->items[actual_index].image_item_preview.image != NULL) ? 50
                                                                         : 10;
  ...
  if (menu->items[*selected].image_item_preview.image != NULL) {
    ...
    my_draw_bitmap_horizontal(oled, img_x, img_y,
                              img->image[img->img_category][img->img_index],
                              img_w, img_h);
  }
  ...
  int box_x =
      (menu->items[*selected].image_item_preview.image != NULL) ? 46 : 6;
  int box_w = 128 - box_x;
...
// --- STUBBED FUNCTIONS TO DISABLE OLED ---
ssd1306_handle_t oled = NULL;
bool g_menu_is_animating = false;

system_err_t ScreenManagerInit(ssd1306_handle_t *_oled) { return MRS_OK; }
void ScreenDashboard(DataManager_t *data) {}
void ScreenSensors(DataManager_t *data) {}
void ScreenPerformance(DataManager_t *data) {}
void MenuRender_Task(void *pvParameters) {
...
system_err_t SensorRender(PortId_t port, SensorData_t *data) { return MRS_OK; }
system_err_t ScreenWifiConnecting(DataManager_t *data) { return MRS_OK; }
system_err_t ScreenShowMessage(Message_t message) { return MRS_OK; }
system_err_t ScreenShowInformation(const char **lines, size_t n_lines) { return MRS_OK; }
system_err_t ScreenShowDataSensor(const char **field_names, const float *values, const char **units, size_t count) { return MRS_OK; }
system_err_t ScreenMeshRoot(DataManager_t *data) { return MRS_OK; }
system_err_t ScreenShowMeshInformation(DataManager_t *data) { return MRS_OK; }
