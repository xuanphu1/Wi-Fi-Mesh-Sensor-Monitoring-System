...
static system_err_t InternetManager_StartMesh(DataManager_t *data, mesh_role_t role) {
  if (data == NULL) {
    return MRS_ERR_CORE_INVALID_PARAM;
  }

  if (MeshManager_IsStarted() && s_mode == INTERNET_MODE_MESH) {
    if (!MeshManager_SwitchRole(role)) {
      return MRS_ERR_MESHMANAGER_INIT_FAILED;
    }
    ESP_LOGI(TAG, "Mesh already running as %s",
  ...
  MeshManager_StartMesh(data, role);
...
system_err_t InternetManager_SwitchMeshMode(DataManager_t *data, mesh_role_t role) {
  ...
  ESP_LOGI(TAG, "Switch mode: %s -> mesh (%s)",
           InternetManager_ModeName(s_mode),
           role == MESH_ROLE_ROOT ? "root" : "node");
  s_transitioning = true;

  system_err_t ret = InternetManager_StartMesh(data, role);
  if (ret != MRS_OK) {
...
system_err_t InternetManager_SwitchMode(DataManager_t *data, internet_mode_t mode) {
  ...
  case INTERNET_MODE_WIFI:
    ...
    break;
  case INTERNET_MODE_MESH:
    ret = InternetManager_StartMesh(data, MESH_ROLE_NODE);
    break;
  default:
