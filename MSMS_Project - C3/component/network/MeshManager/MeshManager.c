...
static bool mesh_get_netif_ip_info(const char *ifkey, esp_netif_ip_info_t *out)
{
    if (ifkey == NULL || out == NULL) {
        return false;
    }
...
static void mesh_io_ensure_udp_sockets(DataManager_t *dm)
{
    ...
    if (role == MESH_ROLE_NODE) {
        ...
        if (cur_tx < 0) {
            ...
            struct in_addr dest_ia = {.s_addr = ip_be};
            ESP_LOGI(TAG_MESH, "Node UDP socket target: %s:%u",
                     inet_ntoa(dest_ia), (unsigned)port_host);
            if (!published) {
                close(s);
            }
        }
        ...
        if (cur_rx < 0) {
            ...
            bool has_ap_ip = mesh_get_netif_ip_info("WIFI_AP_DEF", &ap_ip);
            bool has_sta_ip = mesh_get_netif_ip_info("WIFI_STA_DEF", &sta_ip);
            char ap_ip_str[16] = "0.0.0.0";
            char sta_ip_str[16] = "0.0.0.0";
            if (has_ap_ip) {
                struct in_addr ap_ia = {.s_addr = ap_ip.ip.addr};
                snprintf(ap_ip_str, sizeof(ap_ip_str), "%s", inet_ntoa(ap_ia));
            }
            if (has_sta_ip) {
                struct in_addr sta_ia = {.s_addr = sta_ip.ip.addr};
                snprintf(sta_ip_str, sizeof(sta_ip_str), "%s", inet_ntoa(sta_ia));
            }
            ESP_LOGI(TAG_MESH,
                     "Root UDP socket bound on 0.0.0.0:%u, AP=%s, STA=%s",
                     (unsigned)CONFIG_UDP_PORT, ap_ip_str, sta_ip_str);
            bool published = false;
            if (xSemaphoreTake(dm->meshIo.sock_mutex, pdMS_TO_TICKS(500)) == pdTRUE) {
...
static void mesh_data_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        if (dm->meshIo.role == MESH_ROLE_NODE) {
            ...
            ESP_LOGI(TAG_MESH, "UDP telemetry %d bytes -> %s:%u (schema v%d)",
                     plen, inet_ntoa(dest_addr.sin_addr),
                     (unsigned)dm->meshIo.udp_tx_port_host,
                     MESH_UDP_JSON_SCHEMA);
