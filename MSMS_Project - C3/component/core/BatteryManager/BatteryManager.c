...
// Stub implementation for ESP32-C3
esp_err_t BatteryManager_Init(void) {
  ESP_LOGI(TAG_BATTERY_MANAGER, "BatteryManager stub initialized for non-ESP32 target");
  return ESP_OK;
}

void BatteryManager_StartTask(DataManager_t *data) {
  ESP_LOGI(TAG_BATTERY_MANAGER, "BatteryManager stub task started");
}

float BatteryManager_GetVoltage(void) { return 0.0f; }

uint8_t BatteryManager_GetLevel(void) { return 0; }

uint8_t BatteryManager_GetLevelIndex(void) { return 0; }
...
void BatteryManager_UpdateInfo(BatteryInfo_t *batteryInfo) {
  ...
  batteryInfo->levelPercent = 0;
  batteryInfo->batteryName = "N/A";
