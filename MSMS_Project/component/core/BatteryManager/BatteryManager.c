 ...
 * Reason: legacy driver/adc.h + ADC1_CHANNEL_6/GPIO34 map to classic ESP32 only;
 * ESP32-C6/C3 need esp_adc / adc_oneshot and per-chip channel mapping.
 * Re-enable: change #if 0 to #if 1, migrate to esp_adc for your chip, remove #else stub.
 */
#if 0
...
static esp_err_t battery_adc_init(void) {
  ...
  ESP_LOGI(TAG_BATTERY_MANAGER, "ADC warm-up readings: avg = %.0f", dummy_sum / 10.0f);
...
static void battery_read_task(void *pvParameters) {
  ...
  while (1) {
    ...
    snprintf(battery_text_legacy, sizeof(battery_text_legacy), "%d%% (%.2fV)", level, voltage);
    ESP_LOGI(TAG_BATTERY_MANAGER, "Battery: %d%% (index: %d), Voltage: %.2fV", level, level_index,
             voltage);
    if (data != NULL) {
...
uint8_t BatteryManager_GetLevelIndex(void) { return current_level_index_legacy; }
...
#else /* Stub in use — flip #if 0 to #if 1 at top and drop this #else when ADC is enabled */

static float current_voltage = 0.0f;
static uint8_t current_level = 0;
static uint8_t current_level_index = 0;
static char battery_text[32] = "N/A";

esp_err_t BatteryManager_Init(void) {
  ESP_LOGW(TAG_BATTERY_MANAGER, "Battery ADC disabled (stub)");
  return ESP_OK;
}

void BatteryManager_StartTask(DataManager_t *data) {
  (void)data;
  ESP_LOGW(TAG_BATTERY_MANAGER, "Battery task disabled (stub)");
}

float BatteryManager_GetVoltage(void) { return current_voltage; }

uint8_t BatteryManager_GetLevel(void) { return current_level; }

uint8_t BatteryManager_GetLevelIndex(void) { return current_level_index; }
