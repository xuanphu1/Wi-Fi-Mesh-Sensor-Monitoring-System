...
esp_err_t
_esp_bridge_netif_list_add(esp_netif_t *netif,
...
static void mesh_gateway_stats_note(DataManager_t *dm, bool queued) {
  ...
  if (dm != NULL && dm->meshIo.gateway_rx_queue != NULL) {
    ...
    queue_total =
        queue_used + uxQueueSpacesAvailable(dm->meshIo.gateway_rx_queue);
  }

  ESP_LOGI(TAG_MESH,
           "Gateway RX: udp=%" PRIu32 ", queued=%" PRIu32 ", dropped=%" PRIu32
           ", queue=%u/%u",
           s_gateway_last_stats.udp_rx_frames,
           s_gateway_last_stats.queued_frames,
           s_gateway_last_stats.dropped_frames, (unsigned)queue_used,
...
/**
 * Trong mutex chá»‰ snap fd hiá»‡n táº¡i vÃ  gÃ¡n -1; caller
 * gá»i close() bÃªn ngoÃ i. out_tx/out_rx cÃ³ thá»ƒ NULL náº¿u
 * khÃ´ng cáº§n.
 */
 ...
 * Mutex chá»‰ dÃ¹ng Ä‘á»ƒ Ä‘á»c snap / xuáº¥t báº£n fd
 * má»›i; socket/bind/setsockopt/close ngoÃ i mutex.
 */
static void mesh_io_ensure_udp_sockets(DataManager_t *dm) {
  ...
  if (role == MESH_ROLE_NODE) {
    ...
    if (cur_tx < 0) {
      ...
      if (s < 0) {
        ErrorCodes_PushError(dm->error_code, DATA_MANAGER_ERROR_CAPACITY,
                             MRS_FAIL);
 ...
 */
bool MeshManager_IsConnected(void) { return esp_mesh_lite_get_level() > 0; }

bool MeshManager_IsStarted(void) { return s_mesh_started; }
 ...
 * ---------------------------------------------------------------------------
 *
 * Thứ tự key giữ cố định: v -> seq -> n -> M -> t -> ver -> err -> p.
 *
 ...
 *   - Dùng phân biệt vị trí trong mesh (root/child khác nhau theo stack
 * mesh-lite).
 *
 * M - MAC Address (string, HEX:HEX...)
 *   - Lấy từ esp_read_mac(ESP_MAC_WIFI_STA).
 *   - Khớp định danh cố định với phần cứng thiết bị.
 *   - Giữ chuỗi mặc định (00:00:00:00:00:00) nếu đọc lỗi.
 *
 * t - Local RTC/System time (string, ISO 8601)
 *   - Định dạng: YYYY-MM-DDTHH:MM:SS.
 *   - Lấy từ time()/localtime_r(). Nếu chưa có thời gian hợp lệ thì giữ mặc
 * định "1970-01-01T00:00:00".
 *
 ...
 *   Phía nhận dùng sensor_registry / sensor_type_to_name() hoặc bảng tra cố
 * định trùng firmware để suy ra tên + số field + ý nghĩa từng v[i].
 *
 ...
 * @return Độ dài payload (strlen), hoặc -1 nếu buf/cap không hợp lệ hoặc ghi
 * lỗi.
 */
static int mesh_build_udp_telemetry_json(char *buf, size_t cap,
                                         DataManager_t *dm) {
  if (buf == NULL || cap < 64) {
    return -1;
  }

  char id_str[18] = "00:00:00:00:00:00";
  char time_iso[20] = "1970-01-01T00:00:00";
  char ver_str[16] = "0.0.0";
  if (dm != NULL) {
    snprintf(ver_str, sizeof(ver_str), "%u.%u.%u", (unsigned)dm->version[0],
             (unsigned)dm->version[1], (unsigned)dm->version[2]);
    ESP_LOGI(TAG_MESH, "Telemetry version: %s", ver_str);
    // mesh_log_error_array(dm);
  }
  uint8_t mac[6];
  if (esp_read_mac(mac, ESP_MAC_WIFI_STA) == ESP_OK) {
    snprintf(id_str, sizeof(id_str), "%02X:%02X:%02X:%02X:%02X:%02X", mac[0],
             mac[1], mac[2], mac[3], mac[4], mac[5]);
  }

  if (TimeManager_GetTimestampStr(time_iso, sizeof(time_iso)) != ESP_OK) {
    time_t now = time(NULL);
    struct tm tm_local;
    if (localtime_r(&now, &tm_local) != NULL) {
      if (strftime(time_iso, sizeof(time_iso), "%Y-%m-%dT%H:%M:%S",
                   &tm_local) == 0) {
  ...
  offset +=
      snprintf(buf + offset, cap - (size_t)offset,
               "{\"v\":%d,\"seq\":%" PRIu32
               ",\"n\":%d,\"M\":\"%s\",\"t\":\"%s\",\"ver\":\"%s\",\"err\":[",
               MESH_UDP_JSON_SCHEMA, seq, esp_mesh_lite_get_level(), id_str,
               time_iso, ver_str);
  ...
  if (dm != NULL) {
    for (size_t i = 0; i < DATA_MANAGER_ERROR_CAPACITY; i++) {
      ...
      int w = snprintf(buf + offset, rem, "%s\"0x%02X\"", first_err ? "" : ",",
                       (unsigned)(code & 0xFFU));
      if (w < 0 || (size_t)w >= rem) {
  ...
  if (dm != NULL) {
    for (PortId_t port = 0; port < NUM_PORTS; port++) {
      ...
      if (ret != MRS_OK) {
        ESP_LOGW(TAG_MESH, "Port %d: read failed: %s", port,
...
/**
 * @brief Mesh link + cáº¥u hÃ¬nh UDP: tráº¡ng thÃ¡i káº¿t ná»‘i,
 * esp_mesh_lite_connect khi rá»›t, má»Ÿ/Ä‘Ã³ng socket node (tx) hoáº·c root
 * (rx+bind) theo vai trÃ².
 *
...
/**
 * @brief Chá»‰ truyá»n dá»¯ liá»‡u: node sendto telemetry; root
 * recvfrom â†’ gateway_rx_queue. Socket Ä‘Ã£ Ä‘Æ°á»£c mesh_connection_task má»Ÿ
 * vÃ  cáº¥u hÃ¬nh.
 *
 * @param pvParameters DataManager_t *.
 */
  ...
  for (;;) {
    ...
    if (dm->meshIo.role == MESH_ROLE_NODE) {
      ...
      if (esp_mesh_lite_get_level() <= 1) {
        ESP_LOGW(
            TAG_MESH,
            "Mesh is not joined as child yet (layer=%d), skip UDP telemetry",
      ...
      if (node_tx_sock < 0) {
        node_tx_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
        if (node_tx_sock < 0) {
      ...
      ESP_LOGI(TAG_MESH, "UDP telemetry %d bytes -> %s:%d (schema v%d)", plen,
      ...
      for (int retries = 0; retries < 5; retries++) {
        ...
        if (errno == ENOMEM) {           // errno 12
          vTaskDelay(pdMS_TO_TICKS(50)); // Wait for TX buffer to clear
        } else {
      ...
      if (err < 0) {
        ESP_LOGW(TAG_MESH,
                 "Error occurred during sending: errno %d (TX buffer full)",
                 errno);
      } else {
    ...
    while (dm->meshIo.role == MESH_ROLE_ROOT) {
      ...
      if (recv_len > 0 && dm->meshIo.gateway_rx_queue != NULL) {
        ...
        if (!queued) {
          ESP_LOGW(TAG_MESH, "gateway_rx_queue full, drop UDP frame (%d bytes)",
                   recv_len);
        }
      }
    }
...
/**
 * @brief Log Ä‘á»‹nh ká»³: kÃªnh WiFi, layer mesh, MAC STA, BSSID/RSSI parent,
 * heap, STA con cá»§a AP, danh sÃ¡ch node (náº¿u báº­t report).
 *
 * @note Cháº¡y trong task FreeRTOS Timer Service; náº¿u panic/stack Tmr Svc,
 * tÄƒng CONFIG_FREERTOS_TIMER_TASK_STACK_DEPTH trong menuconfig.
 */
static void mesh_print_system_info_timercb(TimerHandle_t timer) {
  ...
  ESP_LOGI(TAG_MESH,
           "System info: ch=%u, layer=%d, self mac: " MACSTR
...
/**
 * @brief Apply empty STA cfg and Kconfig SoftAP parameters through iot_bridge.
 */
static void mesh_wifi_init(void) {
  // Match ESP-MESH-LITE no-router examples: configure WiFi through iot_bridge
  // so bridge and mesh-lite see the same STA/AP settings.
  wifi_config_t wifi_config;
  memset(&wifi_config, 0x0, sizeof(wifi_config_t));
  wifi_config.sta.pmf_cfg.capable = false;
  wifi_config.sta.pmf_cfg.required = false;
  esp_err_t err = esp_bridge_wifi_set_config(WIFI_IF_STA, &wifi_config);
  if (err != ESP_OK) {
    ESP_LOGW(TAG_MESH, "esp_bridge_wifi_set_config(STA) failed: %s",
             esp_err_to_name(err));
  }
  ...
  err = esp_bridge_wifi_set_config(WIFI_IF_AP, &wifi_softap_config);
  if (err != ESP_OK) {
    ESP_LOGW(TAG_MESH, "esp_bridge_wifi_set_config(AP) failed: %s",
             esp_err_to_name(err));
  }
  ESP_LOGI(TAG_MESH, "Mesh WiFi configured: channel=%d", CONFIG_MESH_CHANNEL);
}
static esp_err_t mesh_wifi_start(void) {
  wifi_mode_t mode = WIFI_MODE_NULL;
  esp_err_t err = esp_wifi_get_mode(&mode);
  if (err != ESP_OK) {
    ESP_LOGW(TAG_MESH, "esp_wifi_get_mode failed before start: %s",
             esp_err_to_name(err));
    return err;
  }
  ...
  if (mode == WIFI_MODE_NULL) {
    ...
    if (err != ESP_OK) {
      ESP_LOGW(TAG_MESH, "esp_wifi_set_mode(APSTA) failed: %s",
               esp_err_to_name(err));
      return err;
    }
  }
...
static esp_err_t
mesh_set_softap_ip_no_napt(esp_netif_t *netif,
  ...
  if (err != ESP_OK) {
    ESP_LOGW(TAG_MESH, "esp_netif_set_dns_info failed: %s",
             esp_err_to_name(err));
  }
...
static esp_err_t mesh_bridge_create_softap_netif_no_deauth(void) {
  ...
  esp_err_t err = esp_netif_dhcps_option(
      wifi_netif, ESP_NETIF_OP_SET, ESP_NETIF_DOMAIN_NAME_SERVER,
      &dhcps_dns_value, sizeof(dhcps_dns_value));
  if (err != ESP_OK) {
    ESP_LOGW(TAG_MESH, "esp_netif_dhcps_option failed: %s",
             esp_err_to_name(err));
  }
...
static void __attribute__((unused)) mesh_bridge_create_netifs_no_deauth(void) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG_MESH, "Create mesh SoftAP bridge netif failed: %s",
             esp_err_to_name(err));
  }
#endif
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG_MESH, "Create mesh STA bridge netif failed: %s",
             esp_err_to_name(err));
  }
#if defined(CONFIG_BRIDGE_WIFI_PMF_DISABLE)
...
/**
 * @brief Load SoftAP SSID/password from NVS or MAC-suffixed defaults, push to
 * mesh-lite.
 */
...
/**
 * @brief One-shot bootstrap: netif, mesh_lite init/start, UDP task, debug
 * timer.
 *
 * @param data Application state; stores mesh client task handle in
 * TaskHandle_Array.
 */
void MeshManager_ResetState(void) {
  if (s_mesh_started) {
    ESP_LOGI(TAG_MESH,
             "Skip esp_mesh_lite_disconnect to keep reconnect available");
  }
...
bool MeshManager_SwitchRole(mesh_role_t role) {
  ...
  if (s_data_manager->meshIo.role == role) {
    ESP_LOGI(TAG_MESH, "SwitchRole: already %s",
             role == MESH_ROLE_ROOT ? "root" : "node");
    return true;
  }

  if (role == MESH_ROLE_ROOT) {
    /* mesh_data_task chuyá»ƒn sang nhÃ¡nh root (Ä‘Ã³ng socket node / má»Ÿ
     * bind). */
    ...
    if (err != ESP_OK && err != ESP_ERR_WIFI_NOT_CONNECT &&
      ...
      ESP_LOGW(TAG_MESH, "esp_wifi_disconnect failed: %s",
               esp_err_to_name(err));
    }
    return true;
  }
...
void MeshManager_StartMesh(DataManager_t *data, mesh_role_t mesh_role) {
  ...
  ESP_LOGI(TAG_MESH, "Starting mesh-lite (%s)...",
           mesh_role == MESH_ROLE_ROOT ? "root" : "child");
  ...
  ESP_LOGI(TAG_MESH,
           "Creating bridge netifs with esp_bridge_create_all_netif()");
  ...
  s_mesh_info_timer =
      xTimerCreate("mesh_print_system_info", pdMS_TO_TICKS(10000), pdTRUE, NULL,
